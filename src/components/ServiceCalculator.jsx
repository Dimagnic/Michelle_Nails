...
const ServiceCalculator = ({ services }) => {
  ...
  return (
    <div style={{ background: '#070d14', border: '1px solid rgba(0,102,204,0.2)', padding: 'clamp(20px,5vw,36px) clamp(16px,4vw,32px)', marginTop: 48, boxSizing: 'border-box', width: '100%' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,200px),1fr))', gap: 10, marginBottom: 28 }}>
        {services.map(s => (
          <label key={s.id} style={{
            display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', overflow: 'hidden',
            ...
            <span style={{ flex: 1, fontSize: 12, color: selected.includes(s.id) ? '#fff' : 'rgba(255,255,255,0.55)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', minWidth: 0 }}>{s.nombre}</span>
